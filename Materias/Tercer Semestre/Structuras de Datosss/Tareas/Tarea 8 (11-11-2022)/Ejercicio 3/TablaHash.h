...
int TablaHash::resolucionPorDobleDireccionDispersa(int clave){
    int hash1 = (hashModular(clave)); //El valor de la primera funcion de hash
    int indiceFinal = hash1; //El valor que se va a devolver, es el indice final
    bool posicionEncontrada = false;
    ...
    if(!tablaLlena()){
        while( !posicionEncontrada ){
            if( tablaHash[indiceFinal].clave != -1){
                contador++;
                indiceFinal = ( hash1 + (contador * hashMultiplicacion(clave)) ) % TAM_TABLA;
            } else {
                posicionEncontrada = true;
            }
        }
    }else{
        cout << "Tabla llena, NO USES EL VALOR QUE DEVUELVE" << endl;
        return -1;
    }
    tablaHash[indiceFinal].clave = indiceFinal;
    return indiceFinal;
}
#endif